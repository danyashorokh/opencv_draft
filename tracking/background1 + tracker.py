...
while True: #(run):
    ...
    cv2.imshow('frame3 + mask_team1', np.hstack([frame3, mask_team1]))
    # --------end try hist 1 ----------
    ...
    for i, c in enumerate(cnts):
        ...
        if cv2.contourArea(c) > 50 and cv2.contourArea(c) < 2000: # and cx < 150 and cy > 150:
    ...
    cv2.imshow('frame1 with cnts rects', frame1)
    cv2.imshow('edged backsubstractor', edged)
    # ---------------- end conts ----------------
