...
def filter_by_hist(field, obj_hist):
    ...
    field_hsv = cv2.cvtColor(masked_field.copy(), cv2.COLOR_BGR2HSV)
...
# cv2.imshow("team1 crop color", team1_init_crop_color)
# cv2.waitKey(0)
...
team2_init_box = (366, 137, 27, 46)
# team2_init_box = cv2.selectROI('team2 player', frame, False)
# print(team2_init_box)
# cv2.destroyWindow("team2 player")

team2_init_crop = frame[int(team2_init_box[1]):int(team2_init_box[1]+team2_init_box[3]),
                 int(team2_init_box[0]):int(team2_init_box[0]+team2_init_box[2])]

team2_init_crop_big = cv2.resize(team2_init_crop, (k_crop * team2_init_crop.shape[1],
                                       k_crop * team2_init_crop.shape[0]), interpolation = cv2.INTER_CUBIC)

team2_init_box = (62, 41, 46, 94)
# team2_init_box = cv2.selectROI('team2 color', team2_init_crop_big, False)
# print(team2_init_box)
# cv2.destroyWindow("team2 color")

team2_init_crop_color = team2_init_crop_big[int(team2_init_box[1]):int(team2_init_box[1]+team2_init_box[3]),
                 int(team2_init_box[0]):int(team2_init_box[0]+team2_init_box[2])]

# cv2.imshow("team2 crop color", team2_init_crop_color)
# cv2.waitKey(0)

team2_hsv = cv2.cvtColor(team2_init_crop_color, cv2.COLOR_BGR2HSV)

team2_hist = cv2.calcHist([team2_hsv],[0, 1], None, [180, 256], [0, 180, 0, 256])
# plt.plot(team2_hist)
# plt.show()

# normalize histogram and apply backprojection
cv2.normalize(team2_hist, team2_hist, 0, 255, cv2.NORM_MINMAX)


mask_team2 = filter_by_hist(masked_field, team2_hist)


cv2.imshow("mask_teams", np.hstack([mask_team1, mask_team2]))
cv2.waitKey(0)
